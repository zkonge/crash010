...
impl Password {
    ...
    pub fn generate_key(
        ...
        unsafe {
            ...
            self.data[8] = encoded_valid_duration[1] ^ self.data[4];
            self.data[9] = encoded_valid_duration[2] ^ self.data[5];
        }
    ...
    fn encode_expire_date(daystamp_of_expiration: u32, seed: u32) -> Result<u32, ()> {
        if daystamp_of_expiration >= 0x1000000 {
            return Err(());
        }

        let mut result: u32 = daystamp_of_expiration * 0x11;
        while result < 0xFF000000 {
            result += 0x1000000;
        }

        loop {
            let mut temp: u32 = result;
            temp ^= 0xFFE53167;
            temp += 0x2C175;
            temp ^= 0x22C078;
            temp ^= seed;
    ...
    fn encode_license_count(generate_count: u16) -> Result<u16, ()> {
        if generate_count > 1000 || generate_count == 0 {
            return Err(());
        }

        let mut ret: u16 = generate_count * 11;
        ret ^= 0x3421;
        ret -= 0x4d30;
        ret ^= 0x7892;
    ...
    fn get_hash(
        ...
        let mut vec2: u8 = 0;
        for c in username.to_uppercase().as_bytes() {
            let cur = *c as usize;
            let temp: u32 = result + TABLE[cur];
            if is_registration_version {
                result = TABLE[(cur + 0x2F) % 256] * (temp ^ TABLE[(cur + 0xD) % 256])
                    + TABLE[vec0 as usize]
                    + TABLE[vec1 as usize]
                    + TABLE[vec2 as usize];
                vec2 += 19;
            } else {
                result = TABLE[(cur + 0x17) % 256] * (temp ^ TABLE[(cur + 0x3F) % 256])
                    + TABLE[vec2 as usize]
                    + TABLE[vec1 as usize]
                    + TABLE[vec0 as usize];
                vec2 += 7;
...
mod test {
    ...
    #[test]
    fn test_encode_expire_date() {
        assert_eq!(Password::encode_expire_date(0, 0x5B8C27), Ok(10403459));
        assert_eq!(Password::encode_expire_date(0x1000001, 0x5B8C27), Err(()));
    }

    #[test]
    fn test_license_count() {
        assert_eq!(Password::encode_license_count(1000), Ok(43323));
        assert_eq!(Password::encode_license_count(233), Ok(34912));
        assert_eq!(Password::encode_license_count(0), Err(()));
        assert_eq!(Password::encode_license_count(1001), Err(()));
    }
